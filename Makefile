...
fmt: $(GOIMPORTS) ## Run go fmt against code.
	$(GOIMPORTS) -local my.domain/inventory -w .
	go fmt ./...
...
CONTROL_CLUSTER_NAME ?= inventory-management
TIMEOUT ?= 10m
NUM_NODES ?= 6
...
.PHONY: fv
fv: $(KUBECTL) $(GINKGO) ## Run inventory Controller tests using existing cluster
...
deploy-inventory: $(KUSTOMIZE)
	# Load inventory image into cluster
	@echo 'Load inventory image into cluster'
	$(MAKE) load-image
	# Install k8s-inventory components
	@echo 'Install k8s-inventory components'
...
helm-test: KIND_CLUSTER ?= k8s-inventory-chart
helm-test: $(KIND)
